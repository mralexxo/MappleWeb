jQuery(document).ready(function(){
	var productCustomization = $('.barraBlanca'),
		cart = $('.cd-cart'),
		animating = false;
	...
	function initCustomization(items) {
		items.each(function(){
			...
			//detect click on the añadir button
	...
	function updateSlider(actual, index) {
		var slider = actual.parent('.barraBlanca').prev('a').children('.bordes'),
	...
	function resetCustomization(selectOptions) {
		...
		selectOptions.siblings('[data-type="select"]').removeClass('is-open').end().parents('.producto').addClass('hover').parent('li').siblings('li').find('.producto').removeClass('hover').end().find('[data-type="select"]').removeClass('is-open');
	}

	function deactivateCustomization() {
		productCustomization.parent('.producto').removeClass('hover').end().find('[data-type="select"]').removeClass('is-open');
	}
